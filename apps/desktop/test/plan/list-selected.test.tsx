...
describe("plan/list-selected", () => {
  it("marks exactly one card selected", () => {
    ...
    const marked = getByTestId("plan-list").querySelectorAll(
      '[aria-selected="true"]',
    );
    expect(marked.length).toBe(1);
    expect(marked[0].getAttribute("data-testid")).toBe(
      `plan-card-${selected.id}`,
    );
  });
  ...
  it("carries the circle-notch on an in-progress card", () => {
    const { getByTestId } = mount();
    expect(
      getByTestId(`plan-card-${selected.id}`)
        .querySelector("use")!
        .getAttribute("href"),
  ...
  it("shows the step line", () => {
    const { getByTestId } = mount();
    expect(getByTestId(`plan-card-step-${selected.id}`).textContent).toBe(
      selected.stepLine,
    );
  });
  ...
  it("right-aligns the project", () => {
    ...
    expect(
      rules(read("screens/screens.css")).find(
        (r) => r.selector === ".plan-card-project",
      )!.body,
