...
describe('plan/one-approval-rule', () => {
  ...
  it('states it from one constant, so the screen and the spec cannot drift', () => {
    expect(ONE_APPROVAL_RULE).toBe('Nothing reaches the board until one approval at the end.')
  })
  ...
  it('offers exactly one approve action on the whole screen', () => {
    ...
    const approvers = [...container.querySelectorAll('button')].filter((b) =>
      b.textContent?.toLowerCase().includes('approve'),
    )
    expect(approvers.length).toBe(1)
    expect(approvers[0].getAttribute('data-testid')).toBe('recommendation-approve')
  })
})
