...
describe("plan/list-approved", () => {
  it("uses muted semantic color for approved cards", () => {
    ...
    for (const plan of approved) {
      expect(getByTestId(`plan-card-${plan.id}`).className, plan.id).toContain(
        "plan-card-approved",
      );
    }
    expect(
      rules(read("screens/screens.css")).find(
        (r) => r.selector === ".plan-card-approved",
      )!.body,
  ...
  it("shows no spinner on an approved card — it is finished, not running", () => {
    const { getByTestId } = mount();
    expect(
      getByTestId(`plan-card-${approved[0].id}`).querySelector("use"),
    ).toBe(null);
  });
