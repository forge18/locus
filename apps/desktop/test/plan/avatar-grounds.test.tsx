...
describe("plan/avatar-grounds", () => {
  it("grounds an agent avatar on --blue", () => {
    ...
    expect(getByTestId(`msg-avatar-${agent.id}`).className).not.toContain(
      "msg-avatar-auditor",
    );
    expect(rule(".msg-avatar").body).toContain(
  ...
  it("grounds the auditor on the deep amber, which is its own colour", () => {
    ...
    expect(getByTestId(`msg-avatar-${auditor.id}`).className).toContain(
      "msg-avatar-auditor",
    );
    expect(rule(".msg-avatar-auditor").body).toContain(
      "background: var(--action-attention-deep)",
    );
  });
