...
const scale = () =>
  Object.fromEntries(
    [...read('styles/tokens.css').matchAll(/(--t-[a-z-]+): *([\d.]+)px/g)].map((m) => [m[1], px(m[2])]),
  )
...
describe('type-scale', () => {
    ...
    for (const [file, contents] of allSource()) {
      const source =
        file === 'shell/shell.css'
          ...
          : contents
      for (const w of declarations(source, 'font-weight').map((x) => x.replace(/['"]/g, ''))) {
        expect(['400', '500', 'normal', 'inherit'], `${file}: unexpected weight ${w}`).toContain(w)
      }
    }
    const badge = read('shell/shell.css').match(/\.rail-badge \{[\s\S]*?\n\}/)![0]
  ...
  it('reads every size from the scale, so the whole thing moves in one edit', () => {
    for (const [file, contents] of allSource()) {
      ...
      expect(contents, `${file} hardcodes a font size`).not.toMatch(/font-size: *[\d.]+px/)
    }
  })
