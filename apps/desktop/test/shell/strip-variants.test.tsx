...
describe("shell/strip-variants", () => {
  it("borders a stuck card in --bad", () => {
    ...
    expect(getByTestId("strip-card-st-stuck").className).toContain(
      "strip-card-stuck",
    );
  ...
  it("marks a shell card with the terminal glyph", () => {
    const { getByTestId } = render(() => <Strip cards={[SHELL]} />);
    expect(
      getByTestId("strip-card-st-shell")
        .querySelector("use")!
        .getAttribute("href"),
    ).toBe("#ph-terminal-window");
  });

  it('reads "no agent · no cost" on a shell, because it is neither', () => {
    const { getByTestId } = render(() => <Strip cards={[SHELL]} />);
    expect(
      getByTestId("strip-card-st-shell").querySelector(".strip-card-bottom")!
        .textContent,
    ).toBe("no agent · no cost");
  });
