...
describe('shell/tabbar-locator', () => {
  it('shows the current view locator on the right', () => {
    ...
    expect(getByTestId('tabbar-locator').textContent).toContain('tapestry/run/8f21')
  })

  it('sets it in mono, pushed right', () => {
    const rule = rules(read('shell/shell.css')).find((r) => r.selector === '.tabbar-locator')!
  ...
  it('offers the detach affordance only when detaching is possible', () => {
    const without = render(() => <TabBar view="runs" onNavigate={() => {}} locator="x" />)
    expect(without.getByTestId('tabbar-locator').querySelector('use')).toBe(null)

    const withDetach = render(() => (
      <TabBar view="runs" onNavigate={() => {}} locator="x" onDetach={() => {}} />
    ))
    expect(
      withDetach.getByTestId('tabbar-locator').querySelector('use')!.getAttribute('href'),
  ...
  it('detaches through the caller — a second window is never opened here', () => {
    ...
    const { getByLabelText } = render(() => (
      <TabBar view="runs" onNavigate={() => {}} locator="x" onDetach={() => detached++} />
    ))
