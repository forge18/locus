...
describe('shell/rail-foot', () => {
  it('carries the branch and account glyphs', () => {
    const { getByTestId } = render(() => <Rail view="inbox" onNavigate={() => {}} inboxCount={0} />)
    expect(
      [...getByTestId('rail-foot').querySelectorAll('use')].map((u) => u.getAttribute('href')),
    ).toEqual(['#ph-git-branch', '#ph-user-circle'])
  })

  it('sets them in --mu2, below the inactive items', () => {
    const rule = rules(read('shell/shell.css')).find((r) => r.selector === '.rail-foot')!
