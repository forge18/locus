...
describe("shell/rail-seven-items", () => {
  ...
  it("gives each its Phosphor glyph at 19px over a 9.5px label", () => {
    ...
    expect(icons.map((i) => i.getAttribute("width")).slice(0, 7)).toEqual(
      Array(7).fill("19"),
    );
    expect(
      [...getByTestId("rail").querySelectorAll(".rail-item use")].map((u) =>
        u.getAttribute("href"),
      ),
    ).toEqual([
      "#ph-tray",
    ...
    expect(rule(".rail-item-label")!.body).toContain(
