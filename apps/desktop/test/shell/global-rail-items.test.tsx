...
describe('shell/global-rail-items', () => {
  it('renders every global route and the Inbox response badge', () => {
    const { getByTestId } = render(() => <ProjectRail selectedProject="locus" inboxCount={3} />)
    const global = getByTestId('global-rail-routes')

    expect([...global.querySelectorAll('button')].map((item) => item.firstChild?.textContent)).toEqual(
      V2_GLOBAL_ROUTE_KINDS.map((route) => route.label),
    )
