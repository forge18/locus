...
import { HEADER_NOTE, HarnessesView } from '../../src/screens/workshop/HarnessesView'
...
describe('harnesses/header', () => {
  it('reads "Registered harnesses N", counted from the registry', () => {
    ...
    expect(getByTestId('harnesses-title').textContent).toContain('Registered harnesses')
    expect(getByTestId('harnesses-count').textContent).toBe(String(useHarnessSummary().harnesses))
  })

  it('says mechanism lives in the file and policy lives here', () => {
    const { getByTestId } = mount()
    expect(getByTestId('harnesses-note').textContent).toBe(HEADER_NOTE)
    expect(HEADER_NOTE).toContain('Mechanism lives in the file; policy lives here')
  ...
  it('offers Register a harness as the primary', () => {
    const { getByTestId } = mount()
    expect(getByTestId('harnesses-register').textContent).toContain('Register a harness')
    expect(getByTestId('harnesses-register').className).toContain('btn-primary')
  })
})
