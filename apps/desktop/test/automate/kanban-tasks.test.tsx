...
describe('automate/kanban-tasks', () => {
  it('renders the project task query across fixed Kanban columns', () => {
    ...
    expect(document.querySelectorAll('[data-column]').length).toBe(COLUMN_ORDER.length)
    for (const task of useTasks()) expect(getByTestId(`manage-task-${task.id}`)).toBeTruthy()
  })
})
