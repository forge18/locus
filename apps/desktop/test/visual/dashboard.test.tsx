 ...
 * 02-dashboard.png — NOT a pixel diff. jsdom has no layout engine, so what is
...
describe("visual: dashboard", () => {
  it("has both reference screenshots to conform to", () => {
    ...
    expect(existsSync(resolve(SHOTS, "02-dashboard.png"))).toBe(true);
  });
  ...
  it("inbox: three cards over three resolved rows, in that order", () => {
    const { getByTestId } = mountInbox();
    expect(
      getByTestId("inbox-list").querySelectorAll(".inbox-card").length,
    ).toBe(3);
    expect(
  ...
  it("inbox: the copy the screenshot shows, verbatim", () => {
    ...
    expect(getByTestId("inbox-approve").textContent).toBe(
      "Approve & release the loop",
    );
    expect(getByTestId("inbox-send-back").textContent).toBe(
  ...
  it("status: six metric cards, then the two panels, then the table", () => {
    ...
    expect(
      getByTestId("status-metrics").querySelectorAll(".metric-card").length,
    ).toBe(6);
  });
  ...
  it("status: twelve bars, three attention rows, four project rows", () => {
    ...
    expect(
      getByTestId("wants-attention").querySelectorAll(".attention-row").length,
    ).toBe(3);
    expect(
      getByTestId("project-table").querySelectorAll("tbody tr").length,
    ).toBe(4);
  });
