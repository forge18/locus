...
/**
 * Structural conformance against screenshots/04-develop.png — not a pixel diff.
...
describe('visual: develop', () => {
  it('has the reference screenshot to conform to', () => {
    expect(existsSync(resolve(SHOTS, '04-develop.png'))).toBe(true)
  })

  it('is three columns at 206 / flex / 252 inside the four bands', () => {
    const { getByTestId } = mount()
    for (const part of ['titlebar', 'rail', 'tabbar', 'strip', 'dev-tree', 'dev-editor', 'git-panel']) {
  ...
  it('lights Develop on the rail and draws no tabs', () => {
    const { getByTestId } = mount()
    expect(getByTestId('rail-develop').getAttribute('aria-current')).toBe('true')
  ...
  it('stacks the editor: tab strip, diff, footer', () => {
    ...
    expect([...editor.children].map((c) => c.getAttribute('data-testid'))).toEqual([
      'dev-tabs',
      'diff',
      'dev-footer',
    ])
  ...
  it('shows the diff side by side under two headers', () => {
    ...
    expect(getByTestId('diff-header-right').textContent).toBe('agent/8f21-notify · builder@4')
  ...
  it('stacks the git panel: header, branch, body, footer', () => {
    ...
    expect(order.slice(0, 4)).toEqual(['git-head', 'git-branch-block', 'git-body', 'git-foot'])
  })
