...
/**
 * Structural conformance against screenshots/03-plan.png — not a pixel diff.
...
describe('visual: plan', () => {
  it('has the reference screenshot to conform to', () => {
    expect(existsSync(resolve(SHOTS, '03-plan.png'))).toBe(true)
  })

  it('is three panes around 216 / flex / 296 inside the four bands', () => {
    const { getByTestId } = mount()
    for (const part of ['titlebar', 'rail', 'tabbar', 'strip', 'plan-list', 'plan-convo', 'plan-outputs']) {
      expect(getByTestId(part), part).toBeTruthy()
    }
    expect(rule('screens/screens.css', '.plan-list').body).toContain('clamp(180px, 17%, 260px)')
    expect(rule('screens/screens.css', '.plan-outputs').body).toContain('clamp(240px, 23%, 340px)')
  })
  ...
  it('stacks the conversation: messages, the scope card, then the live line', () => {
    const { getByTestId } = mount()
    const ids = [...getByTestId('plan-messages').children].map((c) => c.getAttribute('data-testid'))
  ...
  it('shows the outputs rail in the drawn order', () => {
    const { getByTestId } = mount()
    expect(
      [...getByTestId('plan-outputs').querySelectorAll('.output-card')].map((c) =>
        c.getAttribute('data-testid'),
      ),
    ).toEqual(['output-spec', 'output-tasks', 'output-tools', 'recommendation'])
  })
  ...
  it('carries the copy the screenshot shows, verbatim', () => {
    ...
    expect(getByTestId('recommendation-approve').textContent).toBe('Approve — 4 tasks to the board')
    expect(getByTestId('plan-acp').textContent).toBe('ACP · session/prompt')
    expect(container.textContent).toContain('interviewer is re-opening question 14 of 14')
  })
