...
/**
 * Structural conformance against screenshots/14-wiki.png — not a pixel diff.
...
describe('visual: wiki', () => {
  it('has the reference screenshot to conform to', () => {
    expect(existsSync(resolve(SHOTS, '14-wiki.png'))).toBe(true)
  })

  it('is three panes around 246 / flex / 284 inside the four bands', () => {
    const { getByTestId } = mount()
    for (const part of ['titlebar', 'rail', 'tabbar', 'strip', 'wiki-tree', 'wiki-article', 'wiki-side']) {
  ...
  it('stacks the article: tag and title, metadata, prose, links out, provenance', () => {
    ...
    const order = [...article.children].map((c) => c.className || c.getAttribute('data-testid'))
  ...
  it('carries the copy the screenshot shows, verbatim', () => {
    ...
    expect(getByTestId('lint-clean').textContent).toContain('153 pages otherwise clean')
