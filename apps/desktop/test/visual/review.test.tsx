...
/**
 * Structural conformance against screenshots 07, 08 and 09 — not a pixel diff.
...
describe('visual: review', () => {
  it('has all three reference screenshots to conform to', () => {
    for (const shot of ['07-review-telemetry.png', '08-review-runs.png', '09-review-artifacts.png']) {
  ...
  it('lights Review with its three tabs, in order', () => {
    ...
    expect([...getByTestId('tabbar-tabs').querySelectorAll('.tab')].map((t) => t.textContent)).toEqual(
      ['Telemetry', 'Runs', 'Artifacts'],
    )
  })

  it('telemetry: search, chips, five cards, the three-column band, the sessions table', () => {
    const { getByTestId } = mount('telemetry', () => <TelemetryView />)
    expect(getByTestId('tm-metrics').querySelectorAll('.metric-card').length).toBe(5)
    expect(getByTestId('tm-sparkline').querySelectorAll('.sparkline-bar').length).toBe(16)
