...
/**
 * Structural conformance against screenshots/05-automate-kanban.png and
 * 06-automate-agents.png — not a pixel diff. jsdom has no layout engine; what is
...
describe('visual: automate', () => {
  it('has both reference screenshots to conform to', () => {
    expect(existsSync(resolve(SHOTS, '05-automate-kanban.png'))).toBe(true)
    expect(existsSync(resolve(SHOTS, '06-automate-agents.png'))).toBe(true)
  })
  ...
  it('kanban: Automate lit, Kanban tab first and selected', () => {
    ...
    const tabs = [...getByTestId('tabbar-tabs').querySelectorAll('.tab')].map((t) => t.textContent)
  ...
  it('kanban: the four card variants the screenshot draws', () => {
    ...
    expect(getByTestId('task-card-t-002').querySelector('use')!.getAttribute('href')).toBe(
      '#ph-prohibit-inset',
    )
  })
  ...
  it('agents: header, transcript, conditional footer, status bar, in that order', () => {
    ...
    const order = [...pane.children].map((c) => c.getAttribute('data-testid') ?? c.className)
