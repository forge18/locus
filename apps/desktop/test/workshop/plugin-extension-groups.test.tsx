...
describe("workshop/plugin-extension-groups", () => {
  it("exposes exactly Plugins and Extensions subgroup locators", () => {
    ...
    expect(view.getByTestId("workshop-plugins-group").textContent).toContain(
      "Plugins",
    );
    expect(view.getByTestId("workshop-extensions-group").textContent).toContain(
      "Extensions",
    );
  });
  ...
  it("keeps plugin links limited to CLI Tool, Harness, and Provider", () => {
    ...
    fireEvent.click(view.getByRole("button", { name: "Extensions / Plugins" }));
    const labels = [
      ...view.getByTestId("workshop-plugin-links").querySelectorAll("button"),
