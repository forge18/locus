...
describe("screens/desktop-mail", () => {
  it("renders the three-pane fixture with the waiting thread selected", () => {
    ...
    expect(view.getByTestId("mail").getAttribute("data-three-pane")).toBe(
      "true",
    );
    expect(threadButton(view, "thread-1").getAttribute("aria-selected")).toBe(
      "true",
    );
    expect(threadButton(view, "thread-1").getAttribute("data-status")).toBe(
      "waiting",
    );
    expect(view.getByTestId("mail-wait-banner").textContent).toContain(
      MAIL_WAIT_BANNER,
    );
  ...
  it("keeps mail scope page-owned and filters threads by project", async () => {
    ...
    const filter = view
      .getByTestId("page-project-filter")
  ...
  it("keeps the reply draft controlled and sends it as mail reply from you", async () => {
    ...
    await fireEvent.input(textarea, {
      target: { value: "Payload stays id-only on my side." },
    });
  ...
  it("refuses an empty reply with an inline error instead of a silent no-op", async () => {
    ...
    expect(view.getByTestId("inline-error-cause").textContent).toContain(
      "Write a reply",
    );
    expect(view.container.querySelectorAll(".mail-message")).toHaveLength(
      before,
    );

    await fireEvent.input(composer(view), {
      target: { value: "now it has a body" },
    });
  ...
  it("keeps the drained fixture thread read-only with its handoff artifact", async () => {
    ...
    expect(threadButton(view, "thread-5").getAttribute("aria-selected")).toBe(
      "true",
    );
    expect(view.getByTestId("mail-handoff-artifact")).toBeTruthy();
    expect(composer(view).disabled).toBe(true);
