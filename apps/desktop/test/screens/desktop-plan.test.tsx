...
describe("screens/desktop-plan", () => {
  it("renders the seven-stage planning workspace with conversation selected", () => {
    const { getByTestId } = mount();

    expect(getByTestId("plan-workspace-tabs").textContent).toContain(
      "Conversation",
    );
    ...
    expect(getByTestId("breadcrumb").children).toHaveLength(7);
  ...
  it("renders the editable spec with stable requirement ids and its inline finding", () => {
    ...
    expect(
      getByTestId("requirement-R-07").querySelector("textarea")?.value,
    ).toContain("verified_at");
    expect(getByTestId("requirement-finding-R-07").textContent).toContain(
      "missed question",
    );
  ...
  it("maps editable tasks to cards and recalculates the card count", () => {
    ...
    expect(
      getByTestId("granularity-spec-carve-outs").getAttribute("aria-checked"),
    ).toBe("true");
