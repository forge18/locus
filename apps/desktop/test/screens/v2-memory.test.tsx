...
describe('screens/v2-memory', () => {
  it('renders the short-term context window with resident prefix and compaction sections', () => {
    ...
    expect(getByTestId('v2-memory-short-term')).toBeTruthy()
  ...
  it('renders durable facts with scope, provenance, decay, and a resolvable contradiction', () => {
    ...
    expect(getByTestId('v2-memory-long-term')).toBeTruthy()
  ...
  it('renders the artifact viewer with review and reference groups plus comment steering', () => {
    ...
    expect(getByTestId('v2-memory-artifacts')).toBeTruthy()
  ...
  it('renders the curated wiki with provenance, graph, contradictions, and lint findings', () => {
    ...
    expect(getByTestId('v2-memory-wiki')).toBeTruthy()
  ...
  it('uses the semantic data ramp for every context and confidence bar', () => {
    ...
    expect(memoryRule('.v2-memory-bar').body).toContain('background: var(--data-1)')
    expect(memoryRule('.v2-memory-bar > span').body).toContain('background: var(--data-3)')
    expect(memoryRule('.v2-memory-confidence > span').body).toContain('background: var(--data-2)')
    expect(memoryRule('.v2-memory-bar > span').body).not.toContain('var(--ac)')
    expect(memoryRule('.v2-memory-confidence > span').body).not.toContain('var(--ac)')
  })
})
