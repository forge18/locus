...
describe("nav/v2-project-scope", () => {
  ...
  it("formats project routes with an explicit project scope and project segment", () => {
    ...
    expect(
      resolveV2Locator("locus://project/loom-db/review-telemetry"),
    ).toEqual({
  ...
  it("rejects routes addressed with the wrong scope or an implicit v1 scope", () => {
    ...
    expect(() => resolveV2Locator("locus://global/plan-conversation")).toThrow(
      /scope:/,
    );
    expect(() => resolveV2Locator("locus://project/tapestry/inbox")).toThrow(
      /scope:/,
    );
