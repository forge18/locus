...
describe('nav/resolver', () => {
  ...
  it('is the entry point the store navigates through', () => {
    ...
    expect(target).toEqual({ view: 'board', params: { project: 'loom-db', taskId: 't-004' } })
  ...
  it('resolves the project out of the path, so a filter is a query not a screen', () => {
    expect(resolve('locus://weaver/board').params.project).toBe('weaver')
    expect(resolve('locus://texere/board').params.project).toBe('texere')
  })
})
