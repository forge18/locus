...
const mount = (view: 'board' | 'telemetry' = 'board') => {
...
describe('nav/detail-in-place', () => {
  ...
  it('leaves the locator on the current view — you have not gone anywhere', async () => {
    ...
    nav.openDetail('locus://weaver/artifact/a-1')
    await waitFor(() => expect(document.querySelector('[data-testid="sheet"]')).not.toBe(null))
    expect(nav.locator()).toBe('locus://tapestry/board')
  })
  ...
  it('resolves the detail through the same resolver as everything else', async () => {
    ...
    await waitFor(() =>
      expect(document.querySelector('[data-testid="detail-body"]')?.textContent).toContain('a-1'),
    )
  })
  ...
  it('renders inside the app root, never as a second window', async () => {
    ...
    nav.openDetail('locus://weaver/artifact/a-1')
    await waitFor(() => expect(document.querySelector('[data-testid="sheet"]')).not.toBe(null))
    expect(document.getElementById('root')!.contains(document.querySelector('[data-testid="sheet"]')!)).toBe(true)
  ...
  it('closes back to where you were', async () => {
    ...
    await waitFor(() => expect(document.querySelector('[data-testid="sheet"]')).not.toBe(null))
    nav.closeDetail()
    await waitFor(() => expect(document.querySelector('[data-testid="sheet"]')).toBe(null))
    expect(nav.view()).toBe('board')
  })
})
