...
it("agent-panel/diff renders added, removed, context, and line gutters", () => {
  ...
  expect(
    view
      .getAllByTestId("agent-diff-row")
      .map((row) => row.getAttribute("data-diff-kind")),
  ).toEqual(["removed", "added", "context"]);
  expect(
    view
      .getByTestId("agent-inline-diff")
      .querySelectorAll(".agent-diff-line-number"),
  ).toHaveLength(6);
});
...
it("agent-panel/diff announces the row state as text, not only the glyph", () => {
  ...
  expect(
    view
      .getAllByTestId("agent-diff-row")
      .map((row) => row.querySelector(".sr-only")?.textContent),
  ).toEqual(["removed", "added", "context"]);
});
