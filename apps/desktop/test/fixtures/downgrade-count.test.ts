...
describe('fixtures/downgrade-count', () => {
  it('reports 33 downgrades across 96 entries', () => {
    expect(DOWNGRADE_COUNT).toBe(33)
    expect(ENTRY_COUNT).toBe(96)
  })
  ...
  it('counts a downgrade only where the file names what was lost', () => {
    const named = HARNESSES.flatMap((h) => h.extensions).filter((e) => e.weakerThanNative)
    expect(named.length).toBe(DOWNGRADE_COUNT)
    for (const e of named) {
      expect(e.weakerThanNative!.length, `${e.type} says nothing`).toBeGreaterThan(10)
    }
  })

  it('has every harness declare all eight extension types', () => {
    for (const h of HARNESSES) {
      expect(h.extensions.map((e) => e.type).sort(), h.name).toEqual([...EXTENSION_TYPES].sort())
    }
  })
  ...
  it('leaves the reference harnesses with nothing lost', () => {
    for (const name of ['claude', 'pi', 'omp']) {
      ...
      expect(h.extensions.filter((e) => e.weakerThanNative).length, name).toBe(0)
    }
  })
})
