...
import { PROJECT_ROWS } from '../../src/fixtures/status'
...
describe('fixtures/usage-unknown-exists', () => {
  ...
  it('carries the unknown through to the rows a screen renders', () => {
    expect(PROJECT_ROWS.some((r) => r.tokensToday === null)).toBe(true)
    expect(SPEND.some((r) => r.tokens === null && r.cacheReadPct === null)).toBe(true)
  })

  it('passes the session usage through to the events that carry it', () => {
    const unknown = SESSIONS.find((s) => s.usage === null)!
    const carrying = eventsFor(unknown.id).filter((e) => e.verb === 'assistant')
