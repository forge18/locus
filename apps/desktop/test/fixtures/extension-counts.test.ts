...
describe('fixtures/extension-counts', () => {
  ...
  it('adds up to the same 33 downgrades the summary reports', () => {
    expect(EXTENSION_COUNTS.reduce((n, c) => n + c.downgraded, 0)).toBe(DOWNGRADE_COUNT)
  })
  ...
  it('shows context as the type nothing downgrades — every harness reads a file', () => {
    expect(EXTENSION_COUNTS.find((c) => c.type === 'context')!.downgraded).toBe(0)
  })
})
