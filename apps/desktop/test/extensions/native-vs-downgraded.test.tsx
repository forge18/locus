...
describe("extensions/native-vs-downgraded", () => {
  it("shows both numbers on every card", () => {
    ...
    for (const count of counts) {
      expect(
        getByTestId(`type-native-${count.type}`).textContent,
        count.type,
      ).toBe(`${count.native} native · ${count.downgraded} downgraded`);
    }
  });
  ...
  it("takes both from the registry, not from the card", () => {
    for (const count of counts) {
      const downgraded = useHarnesses().filter(
        (h) =>
          h.extensions.find((e) => e.type === count.type)!.weakerThanNative,
      ).length;
      expect(count.downgraded, count.type).toBe(downgraded);
      expect(count.native + count.downgraded, count.type).toBe(
        useHarnesses().length,
      );
    }
  });
  ...
  it("leaves context all native, because every harness reads a file", () => {
    const { getByTestId } = mount();
    expect(getByTestId("type-native-context").textContent).toBe(
      "11 native · 0 downgraded",
    );
  });
  ...
  it("marks at least one type as dominated, so the path is exercised", () => {
    ...
    const { getByTestId } = mount();
    expect(
      getByTestId("type-grid").querySelectorAll('[data-dominated="true"]')
        .length,
    ).toBe(dominated.length);
  });
});
