...
describe("extensions/materialization-card", () => {
  it("is headed MATERIALIZATION with an amber hairline", () => {
    ...
    expect(getByTestId("materialization").textContent).toContain(
      "Materialization",
    );
    expect(
  ...
  it("shows three figures", () => {
    const { getByTestId } = mount();
    expect(
      getByTestId("materialization").querySelectorAll(".materialization-figure")
        .length,
    ).toBe(3);
  });
  ...
  it("computes entries and downgrades from the registry", () => {
    const { getByTestId } = mount();
    expect(getByTestId("materialization-entries").textContent).toContain(
      String(summary.entries),
    );
    expect(getByTestId("materialization-downgrades").textContent).toContain(
      String(summary.downgrades),
    );
    expect(summary.entries).toBe(88);
    expect(summary.downgrades).toBe(29);
  });

  it("shows the cache-read rate, the number determinism exists to protect", () => {
    const { getByTestId } = mount();
    expect(getByTestId("materialization-cache").textContent).toContain(
      CACHE_READ_RATE,
    );
  });

  it("reports the generated registry figures", () => {
    const { getByTestId } = mount();
    const text = getByTestId("materialization").textContent!;
    expect(text).toContain(String(summary.entries));
    expect(text).toContain(String(summary.downgrades));
  });
});
