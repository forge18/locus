...
describe('canvas/node-types', () => {
  it('exposes the six executable palette node kinds and no Goal node', () => {
    ...
    for (const node of usePalette()) expect(getByTestId(`wf-chip-${node.kind}`)).toBeTruthy()
    expect(queryByTestId('wf-chip-goal')).toBeNull()
    expect(getByTestId('wf-node-n-cond').getAttribute('data-node-kind')).toBe('condition')
  })
})
