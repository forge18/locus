...
describe('interaction-states', () => {
  it('lifts one surface step on hover and one further on press', () => {
    expect(rule('.interactive')!.body).toContain('background: var(--sf)')
    expect(rule('.interactive:hover')!.body).toContain('background: var(--sf2)')
    expect(rule('.interactive:active')!.body).toContain('background: var(--sf3)')
  })

  it('gives a surface on the deep ground the same ladder, one step down', () => {
    expect(rule('.interactive-deep:hover')!.body).toContain('background: var(--sf)')
    expect(rule('.interactive-deep:active')!.body).toContain('background: var(--sf2)')
  })

  it('draws focus as the accent outline with a 2px offset', () => {
    const focus = rule(':focus-visible')!
    expect(focus.body).toContain('outline: 2px solid var(--ac)')
  ...
  it('expresses selection as an inset ring, never an outer glow', () => {
    ...
    expect(sel.body).toContain('background: var(--sf2)')
