...
const mount = (item = gate, selected = false) =>
  render(() => (
    <InboxCard item={item} selected={selected} onSelect={() => {}} />
  ));
...
describe("inbox/card-variants", () => {
  it("gives a gate the filled seal-check", () => {
    ...
    expect(iconOf(getByTestId(`inbox-card-${gate.id}`))).toBe(
      "#ph-seal-check-fill",
    );
  });
  ...
  it("gives an ask the question mark", () => {
    ...
    expect(getByTestId(`inbox-card-${ask.id}`).textContent).toContain(
      "locus ask",
    );
  });
  ...
  it("gives a guardrail the filled warning-octagon in --bad", () => {
    ...
    expect(card.querySelector("svg")!.getAttribute("style")).toContain(
      "var(--status-danger)",
    );
  });
  ...
  it("marks the selected card in the DOM and paints it --sf2 with the accent ring", () => {
    ...
    expect(
      getByTestId(`inbox-card-${gate.id}`).getAttribute("aria-selected"),
    ).toBe("true");
