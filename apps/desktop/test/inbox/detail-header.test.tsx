...
describe('inbox/detail-header', () => {
  ...
  it('sets the title at 19px/500', () => {
    ...
    expect(rule('.inbox-detail-title')!.body).toContain('font-size: var(--t-title)')
  ...
  it('carries a mono metadata row of locator · agent · role · gate', () => {
    ...
    expect(meta.textContent).toContain('locus://tapestry/plan')
    ...
    expect(rule('.inbox-detail-meta')!.body).toContain('font-family: var(--fm)')
  })

  it('says the gate is the agent when the item is not a human gate', () => {
    const { getByTestId } = mount(useInboxItems()[2])
    expect(getByTestId('inbox-detail-meta').textContent).toContain('Gate: agent')
  })
