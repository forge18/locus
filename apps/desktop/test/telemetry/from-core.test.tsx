...
describe("telemetry/from-core", () => {
  it("renders the full metric set beside facets and normalized-event search", () => {
    ...
    expect(
      getByPlaceholderText("BM25 search over the normalized event log"),
    ).toBeTruthy();
    expect(
      getByTestId("telemetry-metrics").querySelectorAll("[data-metric]").length,
    ).toBe(AT_A_GLANCE_METRICS.length);
  });
});
