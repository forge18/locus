...
describe('telemetry/layout', () => {
  ...
  it('stacks search, chips, metrics, the band, then the table', () => {
    ...
    expect([...getByTestId('telemetry').children].map((c) => c.getAttribute('data-testid'))).toEqual([
      'tm-search',
      'tm-chips',
      'tm-metrics',
      'tm-band',
  ...
  it('carries no per-screen colours', () => {
    expect(read('screens/review/TelemetryView.tsx')).not.toMatch(/#[0-9a-fA-F]{6}\b/)
  })
  ...
  it('is a query over what is already recorded, not new instrumentation', () => {
    ...
    expect(source).toContain("from '../../data/telemetry'")
