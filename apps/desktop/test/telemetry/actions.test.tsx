...
describe("telemetry/actions", () => {
  ...
  it("fills the track from the data ramp, and in --bad where the verb is a problem", () => {
    ...
    expect(rule(".bar-fill-bad").body).toContain(
      "background: var(--status-danger)",
    );
    const { getByTestId } = mount();
    expect(
      getByTestId("action-tool_error").querySelector(".bar-fill")!.className,
    ).toContain("bar-fill-bad");
    expect(
      getByTestId("action-tool_call").querySelector(".bar-fill")!.className,
    ).not.toContain("bar-fill-bad");
  });

  it("right-aligns the count", () => {
    const { getByTestId } = mount();
    expect(
      getByTestId("action-tool_call").querySelector(".bar-count")!.textContent,
    ).toBe("50,796");
  ...
  it("sizes each fill against the busiest verb", () => {
    ...
    expect(
      (
        getByTestId(`action-${top.verb}`).querySelector(
          ".bar-fill",
        ) as HTMLElement
      ).style.width,
    ).toBe("100%");
