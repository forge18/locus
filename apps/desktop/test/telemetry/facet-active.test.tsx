...
describe('telemetry/facet-active', () => {
  it('marks the active facet in the DOM', () => {
    ...
    expect(getByTestId('facet-verify-failed').getAttribute('aria-pressed')).toBe('true')
    expect(getByTestId('facet-verify-passed').getAttribute('aria-pressed')).toBe('false')
  })
  ...
  it('has exactly one active facet in the fixture, matching the filter chip', () => {
    const active = useFacetGroups().flatMap((g) => g.facets).filter((f) => f.active)
