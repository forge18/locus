...
describe("runs/table", () => {
  it("is headed RUNS with the full count, not the loaded one", () => {
    ...
    expect(getByTestId("runs-panel").textContent).toContain(
      `Runs (${useRunCount()})`,
    );
  ...
  it("spaces out the rows it did not render, so the scrollbar is the whole list", () => {
    const { getByTestId } = mount();
    const last = Number(
      getByTestId("runs-table-rows").getAttribute("data-last"),
    );
    expect(
      (getByTestId("virtual-spacer-bottom") as HTMLElement).style.height,
    ).toBe(`${(612 - last) * ROW_HEIGHT}px`);
  });
  ...
  it("renders unavailable cache and spend as unknown rather than deriving them from tokens", () => {
    const row = table().querySelector("tbody tr")!;
    const cells = [...row.querySelectorAll("td")];
    expect(cells[8].textContent).toBe("unknown");
    expect(cells[9].textContent).toBe("unknown");
  });
});
