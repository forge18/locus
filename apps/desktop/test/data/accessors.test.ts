...
const modules = readdirSync(dataDir).filter((f) => f.endsWith('.ts')).sort()

/** Every fixture module a screen reads has to be reachable through an accessor. */
const DATA_SETS = [
  'agent-defs', 'artifacts', 'board', 'core', 'develop', 'extensions', 'harnesses',
  'inbox', 'plan', 'runs', 'sessions', 'settings', 'status', 'strip', 'telemetry',
  'wiki', 'workflow',
]
...
describe('data/accessors', () => {
  ...
  it('returns data from every accessor', async () => {
    ...
    for (const file of modules) {
      ...
      const mod = (await import(`../../src/data/${name}.ts`)) as Record<string, unknown>
      for (const [key, value] of Object.entries(mod)) {
        ...
        const arg = key === 'useEvidence' ? 't-010' : key === 'useArtifactComments' ? 'a-1' : 's-0000'
        results[`${name}.${key}`] = (value as (a?: string) => unknown)(arg)
      }
    }
    expect(Object.keys(results).length).toBeGreaterThanOrEqual(DATA_SETS.length)
  ...
  it('names the command each accessor becomes, so the M1 swap has a target', () => {
    for (const file of modules) {
      ...
      expect(becomes.length, `${file}: ${accessors.length} accessors, ${becomes.length} targets`)
  ...
  it('is the only thing that reads a fixture', () => {
    for (const file of modules) {
      const source = readFileSync(resolve(dataDir, file), 'utf8')
      expect(source, `${file} reads no fixture`).toMatch(/from '\.\.\/(fixtures|types)\//)
    }
  })
  ...
  it('reports the computed harness summary rather than a written-down number', async () => {
    ...
    expect(useHarnessSummary()).toEqual({ harnesses: 12, entries: 96, downgrades: 33 })
  })
})
