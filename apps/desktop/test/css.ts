  ...
  const walk = (dir: string, prefix: string) => {
    for (const e of readdirSync(dir, { withFileTypes: true }).sort((a, b) =>
      ...
      if (e.isDirectory()) walk(p, rel);
        ...
        out.push([rel, readFileSync(p, "utf8")]);
    }
  };
  walk(SRC, "");
  return out;
...
/**
 * Canonicalize equivalent CSS spellings so contract tests check semantics rather
 * than formatter choices: `0.5`/`.5` and spacing inside colour functions.
 */
