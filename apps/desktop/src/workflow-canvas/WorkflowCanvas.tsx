...
interface WorkflowCanvasProps {
  nodes: CanvasNode[];
  edges: CanvasEdge[];
  loop: { x: number; y: number; width: number; height: number; label: string };
      ...
      <For each={props.data.inputs}>
        {(handle, index) => (
          <Handle
            ...
            style={{ top: handleOffset(index(), props.data.inputs.length) }}
          />
        )}
      </For>
      <div class="wf-node-strip" data-testid={`wf-node-strip-${props.id}`}>
        <span class="wf-node-kind">{props.data.kind}</span>
        <span class="wf-node-state" data-testid={`wf-node-state-${props.id}`}>
          {props.data.state}
        </span>
      </div>
      ...
      <For each={props.data.outputs}>
        {(handle, index) => (
          <Handle
            ...
            id={handle}
            data-handle-id={handle}
            style={{ top: handleOffset(index(), props.data.outputs.length) }}
          />
        )}
      </For>
    </div>
...
function flowNodes(nodes: CanvasNode[], events: NormalizedWorkflowEvent[]) {
  const stateByNode = new Map(events.map((event) => [event.nodeId, event.state]));
  ...
  return (
    <div class="wf-canvas" data-testid="wf-canvas">
      ...
      <svg class="wf-edges" data-testid="wf-edges" aria-hidden="true">
        <defs>
          <For each={["arrow-default", "arrow-pass", "arrow-fail", "arrow-loop"]}>
            {(marker) => (
              <marker
                id={marker}
      ...
      <div class="wf-canvas-foot">
        ...
        <Show when={props.events.length > 0}>
          <span class="wf-live-overlay" data-testid="wf-live-overlay" data-event-source="workflow-events">
