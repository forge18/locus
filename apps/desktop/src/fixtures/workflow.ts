...
export const PALETTE: PaletteNode[] = [
  ...
  { kind: 'condition', label: 'Condition', icon: 'arrows-split', tone: 'condition', required: false },
  { kind: 'gate', label: 'Gate', icon: 'hand-palm', tone: 'default', required: false },
  { kind: 'verify', label: 'Verify', icon: 'flag-checkered', tone: 'default', required: true },
]
...
export const NO_MODEL_NOTE = 'No model in the orchestration path — the graph decides'
...
export const CANVAS_NODES: CanvasNode[] = [
  ...
  { id: 'n-verify', kind: 'verify', label: 'cargo test', state: 'failed 1/3', x: 460, y: 230, tone: 'default' },
  { id: 'n-cond', kind: 'condition', label: 'verify.passed', state: 'evaluable', x: 670, y: 165, tone: 'condition' },
  { id: 'n-review', kind: 'agent', label: 'reviewer@2', state: 'idle', x: 700, y: 40, tone: 'default' },
]
...
export const LOOP_GROUP = { x: 440, y: 80, width: 400, height: 210, label: 'loop · max 3' }

/** The four arrow markers the edge layer defines: one per edge tone. */
export const ARROW_MARKERS = ['arrow-default', 'arrow-pass', 'arrow-fail', 'arrow-loop']
...
export const EXPRESSION: Clause[] = [
  ...
  { operand: 'iteration.index', operator: '<', value: '3' },
]

export const COMPILED = 'verify.passed == true and iteration.index < 3'

export const COMPILED_NOTE = 'total · evaluable in the core · reproducible from stored events'
...
export const GUARDRAILS: Guardrail[] = [
  { key: 'max_iterations', label: 'max_iterations', kind: 'stepper', value: '8' },
  { key: 'reflection_before_retry', label: 'reflection before retry', kind: 'toggle', value: 'on' },
  { key: 'kill_and_reassign', label: 'kill & reassign after', kind: 'stepper', value: '3' },
  { key: 'idle_detection', label: 'idle detection', kind: 'value', value: '60s' },
...
export const VALIDATE_LABEL = 'Validate graph'
export const SAVE_LABEL = 'Save workflow'
...
export const WORKFLOW: WorkflowDef = {
  ...
  nodes: CANVAS_NODES.map((n) => ({
    id: n.id,
    kind: n.kind === 'goal' || n.kind === 'task' || n.kind === 'loop' ? 'agent' : (n.kind as never),
    label: n.label,
...
export const GUARDRAIL_TRIPS: GuardrailTrip[] = [
  { id: 'gt-1', executionId: 'ex-1', kind: 'stuck', action: 'kill_and_reassign', at: '2026-08-20T14:28:00Z', detail: 'three iterations on cargo test -p weaver parser::' },
  { id: 'gt-2', executionId: 'ex-1', kind: 'budget', action: 'warn', at: '2026-08-20T12:04:00Z', detail: '102.3k tokens on one task' },
]
