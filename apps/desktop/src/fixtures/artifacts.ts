 ...
 */
export const REVIEW_KINDS: Array<{ kind: ArtifactKind; label: string; icon: string; note: string }> = [
...
export const REFERENCE_KINDS: Array<{ kind: string; label: string; icon: string; note: string }> = [
  { kind: 'finding', label: 'finding', icon: 'magnifying-glass', note: 'an agent reading its own work' },
  { kind: 'payload', label: 'payload', icon: 'database', note: 'a blob with a handle' },
]
...
export const UNIFIED_DIFF: UnifiedRow[] = [
  ...
  { kind: 'context', no: 19, text: '    pub async fn notify(&self, ch: &str, id: Uuid) -> Result<()> {' },
  { kind: 'removed', no: 20, text: '        sqlx::query("SELECT pg_notify($1, $2)")' },
  { kind: 'added', no: 20, text: '        // NOTIFY carries an id only — payload cap is 8000 bytes' },
  { kind: 'added', no: 21, text: '        sqlx::query("SELECT pg_notify($1, $2)")', commented: true },
  { kind: 'removed', no: 21, text: '            .bind(ch).bind(serde_json::to_string(&row)?)' },
  ...
  { kind: 'context', no: 74, text: '    /// Listener reconnects with backoff.' },
  { kind: 'added', no: 75, text: '        let mut backoff = Duration::from_millis(100);' },
]

export const COMMENTS_TITLE = 'Comments steer the agent'

export const LIVE_COMMENT_NOTE = 'run is still live · comment routed into the session'
