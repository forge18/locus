...
const AGENTS = ['builder@4', 'builder@3', 'reviewer@2', 'planner@3', 'auditor@1', 'ingest@2']
const ROLES = ['builder', 'builder', 'reviewer', 'planner', 'auditor', 'ingest']
const PROJECTS = ['p-tapestry', 'p-loom-db', 'p-weaver', 'p-texere']
const REPOS = ['r-tapestry-app', 'r-loom-db', 'r-weaver', 'r-texere']
const STATUSES: SessionStatus[] = ['running', 'running', 'running', 'waiting', 'idle', 'stuck', 'done']
...
/** 300 sessions, which is what the Automate · Agents list is drawn at. */
export const SESSIONS: Session[] = (() => {
  ...
  return Array.from({ length: 300 }, (_, i) => {
    ...
    return {
      ...
      taskId: next() > 0.3 ? `t-${String(Math.floor(next() * 40)).padStart(3, '0')}` : null,
      handedOffFrom: next() > 0.92 ? `s-${String(Math.max(0, i - 3)).padStart(4, '0')}` : null,
...
export const RUNS: Run[] = SESSIONS.flatMap((s, i) => {
  ...
  return s.runIds.map((id) => ({
    id,
    sessionId: s.id,
    status: s.status === 'running' ? ('running' as const) : pick(next, ['passed', 'passed', 'failed', 'aborted'] as const),
      ...
      verb === 'user'
        ...
        : verb === 'thinking'
          ...
          : verb === 'assistant'
            ...
            : undefined,
    tool: verb.startsWith('tool_') ? pick(next, ['read_file', 'edit_file', 'run_command', 'grep']) : undefined,
    args: verb === 'tool_call' ? { path: 'crates/tapestry-core/src/notify.rs' } : undefined,
    usage: verb === 'assistant' || verb === 'session_end' ? session.usage : undefined,
...
export const SESSION_DETAILS: SessionDetail[] = [
  {
    ...
    idleMinutes: 14,
    transcript: [
      { verb: 'session_start', text: 'session_start · weaver · builder@4 · run 3 of 3' },
      { verb: 'user', text: 'Make cargo test -p weaver parser:: pass.' },
      { verb: 'thinking', text: 'The unterminated block case reaches the recovery arm with an empty span.' },
      { verb: 'tool_call', text: 'run_command  cargo test -p weaver parser::' },
      { verb: 'tool_error', text: 'test result: FAILED. 41 passed; 1 failed' },
      { verb: 'assistant', text: 'Same failure as the previous two iterations. Trying a wider span.' },
  ...
  },
  {
    ...
    idleMinutes: 0,
    transcript: [
      { verb: 'session_start', text: 'session_start · tapestry · builder@4 · run 1' },
      { verb: 'user', text: 'Bound the notify channel and count what it drops.' },
      { verb: 'thinking', text: 'Sink has a blanket impl, so the channel rides it without a new bound.' },
      { verb: 'tool_call', text: 'read_file  crates/tapestry-core/src/notify.rs' },
      { verb: 'tool_result', text: '118 lines' },
      { verb: 'assistant', text: 'Adding a Dropped error and a counter, then threading it through spawn.' },
      { verb: 'tool_call', text: 'edit_file  crates/tapestry-core/src/notify.rs' },
  ...
  },
  {
    ...
    idleMinutes: 6,
    transcript: [
      { verb: 'session_start', text: 'session_start · texere · builder@3 · run 2' },
      { verb: 'assistant', text: 'Plan is written. Waiting on the gate before implementation.' },
  ...
  },
  {
    ...
    idleMinutes: 3,
    transcript: [
      { verb: 'session_start', text: 'session_start · loom-db · builder@4 · run 1' },
      { verb: 'assistant', text: 'Asked which migration path to take; nothing to do until that lands.' },
    ],
  },
  {
    id: 'sd-review',
    ...
    idleMinutes: 2,
    transcript: [
      { verb: 'session_start', text: 'session_start · tapestry · reviewer@2 · run 1' },
      ...
      { verb: 'assistant', text: 'Idempotency key is threaded. No double-charge on retry.' },
    ],
  },
]
