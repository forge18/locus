  ...
  return (
    <div class="running-pill-wrap">
      ...
      >
        ...
        <span data-testid="running-count" aria-live="polite">
          {props.running} running
        </span>
        <span data-testid="needs-you-count" aria-live="assertive">
      ...
      <Show when={open()}>
        <div aria-label="Active sessions" class="running-popover" role="dialog">
          <ul data-testid="active-session-list">
            <For each={orderedSessions(props.sessions ?? [])}>
              {(session) => <li>{session.label}</li>}
            </For>
          </ul>
          <button
