    ...
    return (
        ...
        >
            ...
            <Show
                when={expanded()}
                fallback={
                    <p class="agent-user-preview">{eventText(props.event)}</p>
                }
            >
                <Show
                    when={!editing()}
                ...
                <div class="agent-card-actions">
                    ...
                    <button
                        type="button"
                        onClick={() => props.onCopyPrompt?.(draft())}
                    >
                        Copy
                    </button>
                    <button
            ...
            >
                ...
                <Show when={open()}>
                    <pre>
                        {safeJson(
                            props.event.args ??
                                rawObject(props.event.raw.args) ??
                                {},
                        )}
                    </pre>
                    <p>{eventText(props.event)}</p>
                    ...
                    <>
                        <div class="agent-diff" data-testid="agent-inline-diff">
                            <header>
                                <Show
                                    when={diff.path}
                                    fallback={
                                        <span>file path unavailable</span>
                                    }
                                >
                                    {(path) => (
                                        <FileLink
                            ...
                            <Show
                                ...
                                fallback={
                                    <p class="agent-diff-unavailable">
                                        Diff unavailable; inspect the recorded
                        ...
                        <div class="agent-card-actions">
                            ...
                            <button
                                type="button"
                                onClick={() =>
                                    props.onApproveRemaining?.(props.event)
                                }
                            >
                    ...
                    <Switch fallback={<ActivityRow event={event} />}>
                        ...
                        <Match when={event.verb === "assistant"}>
                            <AgentTurn
                                event={event}
                                onOpenFile={props.onOpenFile}
                                onPinCitation={props.onPinCitation}
                            />
                        </Match>
                        <Match when={event.verb === "thinking"}>
                            <ThinkingBlock
                                event={event}
                                display={props.thinkingDisplay}
                                onDisplay={props.onThinkingDisplay}
                            />
                        </Match>
                        <Match
                            when={
                                event.verb === "tool_call" ||
                                event.verb === "tool_result" ||
                                event.verb === "tool_error"
                            }
                        >
                            <ToolCard
                                event={event}
                                display={props.toolCallsDisplay}
                                onOpenFile={props.onOpenFile}
                            />
                        </Match>
                        <Match
                            when={
                                event.verb === "permission_request" &&
                                props.posture === "gated"
                            }
                        >
                            <PermissionCard
                                event={event}
                                posture="gated"
                                resolved={props.resolved.includes(event.id)}
                                onApprove={props.onApprove}
                                onDecline={props.onDecline}
                                onApproveRemaining={props.onApproveRemaining}
                                onOpenFile={props.onOpenFile}
                            />
                        </Match>
