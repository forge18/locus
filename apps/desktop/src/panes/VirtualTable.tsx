...
export function VirtualTable<T>(props: VirtualTableProps<T>) {
  ...
  const loading = () => props.loading === true || props.rows.length < props.total
  const hasError = () => props.error !== undefined
  const empty = () => !hasError() && !loading() && props.rows.length === 0 && props.total === 0
  ...
  const initialLoading = () => state() === 'loading' && props.rows.length === 0
  const errorMessage = () =>
  ...
  return (
    ...
    >
      ...
      <Show when={hasError()}>
        <div data-testid={`${testId()}-error`} style={{ padding: 'var(--g-3) var(--g-4)' }}>
          <InlineError
            cause={errorMessage() || 'The table could not be loaded.'}
            next={props.onRetry ? 'Retry loading this table.' : 'Refresh to try again.'}
            action={
              props.onRetry ? (
      ...
      <Show when={empty()}>
        <div data-testid={`${testId()}-empty`} style={{ 'min-height': `${props.height}px` }}>
          <EmptyPane reason={props.emptyMessage ?? 'No rows to display.'} />
        </div>
      </Show>
      ...
      <Show when={!hasError() && loading() && props.rows.length > 0 && props.rows.length < props.total}>
        <div class="virtual-loading" data-testid={`${testId()}-loading`} role="status">
