import type { ProjectLocalRemote, ProjectRepo, ProjectSetup, ProjectSummary } from "../types/core";
...
export function fetchRepos(projectId: string): Promise<Envelope<ProjectRepo[]>> {
...
export function fetchLocalRemotes(projectId: string): Promise<Envelope<ProjectLocalRemote[]>> {
  return dataProvider().query<ProjectLocalRemote>("local_remotes_list", { projectId });
}

/** One project's harness policy and base context. */
export function fetchProjectSetup(projectId: string): Promise<Envelope<ProjectSetup>> {
