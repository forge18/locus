...
function validNavigation(
  ...
  return {
    ...
    owner: typeof candidate.owner === 'string' ? candidate.owner : fallbackOwner,
  }
}

function decodeStoredNavigation(raw: string | null): PersistedNavigation | null {
  if (!raw) return null
  try {
...
export function createNavStore(options: NavStoreOptions = {}): NavStore {
  ...
  const restored = !options.view && !options.project
    ? readPersistedNavigation()
    : null
  const start = restored
    ? resolve(restored.stack[restored.cursor])
    : fallback
  ...
  if (typeof window !== 'undefined') {
    ...
    window.addEventListener('storage', (event) => {
      if (
        (event.key !== NAV_HISTORY_STORAGE_KEY && event.key !== NAV_STORAGE_KEY) ||
