...
export const KINDS = ['session', 'task', 'artifact', 'page', 'workflow', 'agent'] as const
export type LocatorKind = (typeof KINDS)[number]
export type ViewParams = { project?: string; [key: string]: string | undefined }

const VIEW_IDS = Desktop_FIXTURE_ROUTES.map((route) => route.id) as readonly string[]
const PROJECT_VIEWS = new Set(Desktop_FIXTURE_ROUTES.filter((route) => route.scope === 'project').map((route) => route.id))
const APP_VIEWS = new Set(Desktop_FIXTURE_ROUTES.filter((route) => route.scope === 'app').map((route) => route.id))
const SEGMENT = /^[A-Za-z0-9._@-]+$/
const KIND_SUB: Partial<Record<LocatorKind, string>> = { session: 'run', workflow: 'execution' }
...
function viewFor(kind: LocatorKind, hasSub: boolean): View {
  ...
  if (kind === 'task') return 'manage' as View
...
export function parse(locator: string): Locator {
  if (!locator.startsWith(LOCATOR_SCHEME)) throw new LocatorError(`scheme: expected "${LOCATOR_SCHEME}"`)
  const segments = locator.slice(LOCATOR_SCHEME.length).split('/')
  const [scope, kind, id, ...sub] = segments
  if (!scope || !SEGMENT.test(scope)) throw new LocatorError(`scope: "${scope}" is invalid`)
  ...
  if (isScope(scope)) throw new LocatorError('scope: object locators require a project')
  if (!(KINDS as readonly string[]).includes(kind)) throw new LocatorError(`kind: "${kind}" is not supported`)
  const k = kind as LocatorKind
  if (!id || !SEGMENT.test(id)) throw new LocatorError(`id: "${id}" is invalid`)
  if (k === 'agent' && !/^[A-Za-z0-9._-]+@\d+$/.test(id)) throw new LocatorError(`id: "${id}" is not <name>@<version>`)
  if (!sub.length) return { project: scope, kind: k, id, subId: null }
  const allowed = KIND_SUB[k]
  if (!allowed || sub.length !== 2 || sub[0] !== allowed || !SEGMENT.test(sub[1])) {
...
export function format(view: View, params: ViewParams): string {
  ...
  if (form) {
    ...
    if (kind === 'agent' && id && params.agentVersion) id = `${id}@${params.agentVersion}`
    if (id) {
      if (!project || isScope(project)) throw new LocatorError(`project: object locators require a project`)
      if (subParam === 'runId' && !params[subParam]) return `${LOCATOR_SCHEME}${project}/view/${view}`
      const sub = subParam && params[subParam]
      return `${LOCATOR_SCHEME}${project}/${kind}/${id}${sub ? `/${kind === 'session' ? 'run' : 'execution'}/${sub}` : ''}`
    }
  }
  if (scope === 'project') {
    if (!project || isScope(project)) throw new LocatorError('project: a project-scoped view requires a project')
    return `${LOCATOR_SCHEME}${project}/view/${view}`
  }
  if (project !== undefined) throw new LocatorError(`scope: ${scope} views do not carry a project`)
...
export function resolve(locator: string): NavTarget {
  ...
  if (parsed.kind === 'agent') {
    ...
    const key = parsed.kind === 'session' ? 'sessionId' : parsed.kind === 'artifact' ? 'artifactId' : parsed.kind === 'page' ? 'slug' : parsed.kind === 'workflow' ? 'workflowId' : 'taskId'
    params[key] = parsed.id
  }
  if (parsed.subId) params[parsed.kind === 'session' ? 'runId' : 'executionId'] = parsed.subId
  return { view, params }
}
