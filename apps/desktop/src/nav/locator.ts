...
export function parse(locator: string): Locator {
  if (!locator.startsWith(LOCATOR_SCHEME)) {
    throw new LocatorError(`scheme: expected "${LOCATOR_SCHEME}", got "${locator.split('/')[0]}//"`)
  }
  ...
  if (tail.length === 0) {
    throw new LocatorError('view: a locator addresses a view or an object, and this names neither')
  }

  // View form: locus://<project>/<view>
  if (tail.length === 1) {
    if (!(VIEWS as readonly string[]).includes(tail[0])) {
      throw new LocatorError(`view: "${tail[0]}" is not one of the fourteen views`)
    }
  ...
  if (!allowed) {
    throw new LocatorError(`sub: kind "${kind}" carries no sub-object, got "${sub[0]}"`)
  }
  if (sub.length !== 2) {
    throw new LocatorError(`sub: expected "${allowed}/<id>", got "${sub.join('/')}"`)
  }
  if (sub[0] !== allowed) {
    throw new LocatorError(`sub: kind "${kind}" carries "${allowed}", not "${sub[0]}"`)
  }
...
export function format(view: View, params: ViewParams): string {
  ...
  if (form.kind === 'agent' && id && params.agentVersion) id = `${id}@${params.agentVersion}`
