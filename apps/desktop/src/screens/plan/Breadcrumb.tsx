...
export function stepState(step: string, current: string): StepState {
  const at = PLAN_STEPS.indexOf(current as PlanStep);
  const here = PLAN_STEPS.indexOf(step as PlanStep);
  return here < at ? "done" : here === at ? "current" : "ahead";
}

/** Seven stages, three states, and which is which comes from the plan, not the markup. */
