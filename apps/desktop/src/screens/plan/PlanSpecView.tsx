...
export function PlanSpecView() {
  ...
  const findingText = (requirement: SpecRequirement) =>
    resolved().includes(requirement.id)
      ? "Finding resolved"
      : requirement.finding;
  ...
  return (
    <section class="plan-spec" data-testid="plan-spec">
      <header class="plan-spec-toolbar">
        ...
        <span class="plan-spec-meta">
          desktop · 14 requirements · you edited R-07 at 09:44
        </span>
        <span class="plan-unsaved" data-testid="spec-unsaved">
          unsaved
        </span>
        <div class="plan-spec-actions">
          <Button variant="ghost">Revert</Button>
          <Button variant="secondary">History</Button>
          <Button variant="primary">Save &amp; re-synthesise</Button>
        </div>
      </header>

      <div class="plan-spec-body">
        <article class="plan-spec-document">
          <h2 class="plan-spec-title">
            # Provenance beats recency in memory conflicts
          </h2>
          <p class="plan-spec-intro">
            Edited here, the spec is the artefact the board is built from.
            Requirement ids are stable: a task already on the board keeps
            pointing at the requirement it came from, even after a rewrite.
          </p>
          <h3 class="plan-spec-section">## 3 · Conflict resolution</h3>
          <For each={requirements()}>
            {(requirement) => (
              <div
                class="plan-requirement"
                data-testid={`requirement-${requirement.id}`}
              >
                <label
                  class="plan-requirement-id"
                  for={`requirement-input-${requirement.id}`}
                >
                  {requirement.id}
                </label>
                <div class="plan-requirement-content">
                  ...
                  />
                  <div
                    class="plan-requirement-finding"
                    data-testid={`requirement-finding-${requirement.id}`}
                  >
                    ...
                    {requirement.finding &&
                      !resolved().includes(requirement.id) && (
                        <Button
                          variant="ghost"
                          data-testid={`resolve-finding-${requirement.id}`}
                          onClick={() =>
                            setResolved((ids) => [...ids, requirement.id])
                          }
                        >
          ...
          <Button variant="secondary" class="plan-add-requirement">
            + Add requirement to §3
          </Button>
          <p class="plan-spec-note">
            Saving sends stage 5 back over the changed requirements only.
            Requirements already carried by a card on the board are marked, so
            you can see what a rewrite is about to contradict.
          </p>
        </article>
        <aside
          class="plan-outline"
          aria-label="Spec outline"
