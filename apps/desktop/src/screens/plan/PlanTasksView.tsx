...
export function PlanTasksView() {
  const [granularity, setGranularity] =
    createSignal<PlanGranularity>("spec-carve-outs");
  ...
  const ownsCard = (id: string) =>
    granularity() === "every-task" ||
    (granularity() === "spec-carve-outs" && carvedOut().includes(id));

  const toggleTask = (id: string) => {
    if (granularity() !== "spec-carve-outs") return;
    setCarvedOut((current) =>
      current.includes(id)
  ...
  const cardSummary = () => {
    if (granularity() === "spec")
      return "one card, decomposed by the agent at run time";
    if (granularity() === "every-task")
      return "dependencies carried onto the board";
  ...
  return (
    <section class="plan-tasks" data-testid="plan-tasks">
      <header class="plan-tasks-head">
        <div>
          <span class="plan-stage-label">Step 6 of 7</span>
          <h2>Decompose</h2>
          <span class="plan-stage-note">decided once, before approval</span>
        </div>
        <p>
          A card is what the board schedules and what you watch progress on. Too
          coarse and you cannot see where a run got stuck; too fine and you are
          managing a to-do list instead of a plan.
        </p>
      </header>
      ...
      <div class="plan-tasks-body">
        <section>
          ...
          <div
            class="plan-granularity-options"
            role="radiogroup"
            aria-label="Card granularity"
          >
            <For each={PLAN_GRANULARITY_OPTIONS}>
              {(option) => (
                ...
                >
                  ...
                  <span class="plan-granularity-yield">
                    {option.yield} ·{" "}
                    {option.id === "spec"
                      ? "1 card"
                      : option.id === "every-task"
                        ? `${PLAN_TASKS.length} cards`
                        : `${count()} cards`}
                  </span>
                </button>
              )}
        ...
        <section>
          <div class="plan-task-table-heading">
            ...
            <span>
              {granularity() === "spec"
                ? "all four ride along on the spec card"
                : granularity() === "every-task"
                  ? "every task gets its own card"
                  : `${carvedOut().length} carved out, ${tasks().length - carvedOut().length} riding along`}
            </span>
          ...
          <div class="plan-task-table">
            <div class="plan-task-row plan-task-columns plan-task-table-labels">
              ...
              <span>Estimate</span>
              <span>After</span>
              <span>On the board as</span>
            </div>
            <For each={tasks()}>
              {(task) => (
                <div
                  class="plan-task-row plan-task-columns"
                ...
                >
                  ...
                  <Input
                    mono
                    value={task.role}
                    aria-label={`${task.id} role`}
                    onInput={(event) =>
                      updateTask(task.id, "role", event.currentTarget.value)
                    }
                  />
                  <Input
                    mono
                    value={task.estimate}
                    aria-label={`${task.id} estimate`}
                    onInput={(event) =>
                      updateTask(task.id, "estimate", event.currentTarget.value)
                    }
                  />
                  <Input
                    mono
                    value={task.dependency}
                    aria-label={`${task.id} dependency`}
                    onInput={(event) =>
                      updateTask(
                        task.id,
                        "dependency",
                  ...
                  <button
                    type="button"
                    class="plan-task-card-toggle"
                    data-testid={`task-card-toggle-${task.id}`}
                    disabled={granularity() !== "spec-carve-outs"}
                    onClick={() => toggleTask(task.id)}
                  >
                    {granularity() === "spec"
                      ? "inside the spec card"
                      : ownsCard(task.id)
                        ? "its own card"
                        : "rides on the spec card"}
                  </button>
                </div>
              )}
            </For>
            <footer class="plan-task-summary" data-testid="tasks-cards-summary">
              <span class="plan-card-total" data-testid="task-card-count">
          ...
          <p class="plan-tasks-note">
            Anything not carved out rides along on the spec card, and the agent
            decomposes it at run time. Carve out the ones you want to watch
            separately — usually the long ones and the ones you expect to get
            stuck.
          </p>
        </section>
      </div>
