...
export function ViewerStateFamilies() {
  return <section data-testid="viewer-state-families" aria-live="polite" data-visual-themes="light,dark">{VIEWERS.flatMap((viewer) => STATES.map((state) => <div data-viewer-state={`${viewer}:${state}`} />))}</section>
}
