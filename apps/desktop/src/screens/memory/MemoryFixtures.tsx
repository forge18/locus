...
export function MemoryShortTermFixture() {
  return (
    <MemoryFrame testId="desktop-memory-short-term" route="memory-short-term" contextLayers="resident" compaction="artifact-handles" contextBudget="120k">
          ...
          <For each={sessions}>
            {([title, detail], index) => (
              <div class="desktop-memory-list-item" data-selected={index() === 0 ? 'true' : undefined}>
                <div>{title}</div>
                <small class={index() === 1 ? 'desktop-memory-bad' : ''}>{detail}</small>
              </div>
            )}
          </For>
        </div>
      ...
      <aside class="desktop-memory-right">
        ...
        <section class="desktop-memory-side-card">
          ...
          <p><code class="desktop-memory-bad">102.3k</code> weaver · builder@4</p>
...
export function MemoryLongTermFixture() {
  return (
    <MemoryFrame testId="desktop-memory-long-term" route="memory-long-term" factFixture="long-term" factState="provenance-confidence-decay-contradiction">
      <aside class="desktop-memory-left">
        <header class="desktop-memory-pane-head">
        ...
        <section class="desktop-memory-scope"><Label>Scope</Label><span>#tapestry</span><small>never cross-project</small></section>
        <FactList />
      </aside>
      ...
      <section class="desktop-memory-main">
        ...
        <div class="desktop-memory-scroll">
          <p class="desktop-memory-prose">Postgres truncates a NOTIFY payload above 8000 bytes without raising. The payload carries the row id and the listener re-reads what it names.</p>
          <section>
          ...
          <section>
            ...
            <p class="desktop-memory-note">decay is the forgetting half; curation is the reconciling half</p>
            <div class="desktop-memory-confidence" aria-label="Confidence increased from 0.38 to 0.94">
      ...
      <aside class="desktop-memory-right">
        <section class="desktop-memory-side-card"><Label>This project’s memory</Label><strong>318</strong> facts · <strong class="desktop-memory-ok">146</strong> verified · <strong>1</strong> contradicted</section>
        <section class="desktop-memory-side-card">
        ...
        <section class="desktop-memory-side-card"><Label>Decay</Label><p><code>41</code> fell below recall threshold last night</p><p><code>6</code> promoted on a passing verify</p></section>
        <section class="desktop-memory-side-card"><Label>locus memory explain</Label><code>$ locus memory explain mem-1184<br />recalled 31× · 0.94<br />source: run r-9f21 verify</code></section>
      </aside>
    </MemoryFrame>
  )
}
...
export function MemoryArtifactsFixture() {
  return (
    <MemoryFrame testId="desktop-memory-artifacts" route="memory-artifacts" artifactGroups="review-reference" artifactPreview="comments-review">
      <aside class="desktop-memory-left">
        <Label>Review artifacts</Label>
        <div class="desktop-memory-list">
          <For each={artifacts}>{([title, detail], index) => <div class="desktop-memory-list-item" data-selected={index() === 0 ? 'true' : undefined}><div>{title}</div><small>{detail}</small></div>}</For>
        </div>
        <Label>Reference · never in the inbox</Label>
        <div class="desktop-memory-list"><div class="desktop-memory-list-item">finding · ACP prior art</div><div class="desktop-memory-list-item">payload · 62.4kB fetch</div></div>
      </aside>
      <section class="desktop-memory-main">
        <header class="desktop-memory-title-row"><Tag>diff</Tag><h2>store/notify.rs</h2><code>{ARTIFACT_LOCATOR}</code><small>one viewer per kind · three entry points</small></header>
        <pre class="desktop-memory-diff">@@ -19,6 +19,7 @@ impl Store{`\n`}  pub async fn notify(&self, ch: &str, id: Uuid){`\n`}<ins>+ // NOTIFY carries an id only — cap is 8000 bytes</ins>{`\n`}<del>- .bind(serde_json::to_string(&row)?)</del>{`\n`}<ins>+ .bind(id.to_string())</ins>{`\n`}  .execute(&self.pool).await?;</pre>
      </section>
      <aside class="desktop-memory-right">
        <Label>Comments steer the agent</Label>
        <div class="desktop-memory-comment"><small>you · line 22</small>The listener needs to handle a row deleted between NOTIFY and re-read.</div>
        <div class="desktop-memory-comment" data-live="true"><small>builder@4 · replying</small>Added <code>Ok(None)</code> on a missing row and a test that deletes before the listener wakes.</div>
        <footer class="desktop-memory-comment-form"><textarea placeholder="Comment on line 62…" /><Button variant="primary">Send to session</Button><Button>Resolve</Button></footer>
      </aside>
    </MemoryFrame>
  )
}
