...
const agents = [
  [
    "builder@4",
    "implements one task at a time on a branch",
    "41 runs",
    "running",
  ],
  ...
  [
    "interviewer@3",
    "elicits the plan, asks until it stops learning",
    "9 runs",
    "4h",
  ],
...
export function Analytics() {
  return (
    <div class="project-analytics" data-testid="project-analytics">
      <section>
        <div class="section-title">Carrying this tag</div>
        <div class="project-counter-grid">
          <For each={tagCounters}>
            {([value, label, note]) => (
              <div data-testid="project-tag-counter">
                <strong>{value}</strong>
                <span>{label}</span>
                <small>{note}</small>
              </div>
      ...
      <section class="project-panel">
        <PanelTitle title="Agents" note="6 have run here" />
        <div class="project-agents">
          <For each={agents}>
            {([name, description, runs, state], index) => (
              <div data-testid="project-agent">
                <i
                  classList={{
                    "agent-active": index() === 0,
                    "agent-waiting": index() === 1,
                  }}
                />
                <strong class="mono">{name}</strong>
                <span>{description}</span>
                <small class="mono">{runs}</small>
                <b>{state}</b>
              </div>
            )}
          </For>
        ...
        <p class="project-panel-note">
          Agent definitions are global. What is per-project is which ones have
          run here, and what they learned.
        </p>
      </section>
      <section class="project-panel" data-testid="project-statistics">
        <PanelTitle title="Statistics" note="since 14 Mar 2026 · 160 days" />
        <div class="project-stat-grid">
          <Stat value="160" label="days since first run" />
          <Stat value="312" label="tasks reached the board" />
          <Stat value="287" label="tasks landed" />
          <Stat value="6" label="agents have run here" />
          <Stat value="1,204" label="runs, 78% verified" />
          <Stat value="$1,842" label="spend, all time" />
        </div>
        <div class="project-models">
          ...
          <For each={models}>
            {([model, tokens, cache, spend, width]) => (
              <div data-testid="project-model-row">
                <strong class="mono">{model}</strong>
                <span class="model-token">
                  <i>
                    <em style={{ width }} />
                  </i>
                  <b>{tokens}</b>
          ...
          <footer data-testid="project-analytics-total">
            <span>4 models</span>
            <span>41.7M</span>
            <span>88%</span>
            <span>$1,842</span>
        ...
        <p class="project-panel-note">
          Cache read is the number worth watching — the materialized tree is the
          prompt prefix, so a drop means something upstream stopped being
          byte-deterministic.
        </p>
      </section>
    </div>
