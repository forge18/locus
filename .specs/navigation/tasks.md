...
| 9 | Rail click → the category's documented first view | 8 | `pnpm -C apps/desktop test -- nav/rail-first-view` |
| 10 | Tab click → that view, instant, no transition | 8 | `pnpm -C apps/desktop test -- nav/tab-click` |
| 11 | `agents` as an Extensions drill-down: Extensions stays lit, back link renders | 8 | `pnpm -C apps/desktop test -- nav/agents-is-drilldown` |
| 12 | Assert no Agents tab exists in the Workshop tab set | 2,11 | `pnpm -C apps/desktop test -- nav/no-workshop-agents-tab` |
| 13 | Assert the category list is exactly seven and closed to extension at runtime | 1 | `pnpm -C apps/desktop test -- nav/seven-closed` |
