...
Adopt `docs/design_handoff_locus_v2/` without porting its HTML, standalone bundle, or `support.js`.
The implementation remains SolidJS/TypeScript in `apps/desktop` and Rust in `locus-core`. This feature
replaces v1 fixture contracts with v2 fixtures and makes the architectural changes explicit before
runtime work depends on them.

## Governed by

- `PLAN.md` §Decisions and §v2 desktop revision
- `DESIGN.md`
- `docs/design_handoff_locus_v2/README.md`, its HTML reference, standalone bundle, `support.js`, and
  all 31 screenshots
...
The v2 shell has a 42px title bar, a 212px rail, a selected-project card, and a running-agent pill
...
Fixture coverage is the 31 v2 screens named in the handoff README. Screens use the semantic v2 tokens:
...
theme layering, root preference, and theme acceptance live in `theme-system`; M0.6 ships both v2 Dark
...
| Existing feature | v2 replacement |
| --- | --- |
| `app-shell`, `navigation` | project-scoped rail and running-agent popover |
| `design-system`, `fixtures`, all `screens-*` | v2 tokens, theme layering, Dark/Light fixture set, and 31-screen fixture set |
...
| `memory`, `tool-compaction`, `dashboard-metrics` | v2 Memory and Project Analytics viewers |
| `marketplace-index`, `marketplace-installer` | Minisign-trusted CLI catalog and image installation contract |

## Acceptance

1. Every v2 screen has a named fixture route; v1-only screens are unreachable.
