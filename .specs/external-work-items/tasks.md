...
| # | Task | Deps | verify |
| --- | --- | --- | --- |
| 1 | Define normalized external-work-item identity, snapshot, and completion capabilities | — | `cargo test -p locus-core work_item::contract_types` |
| 2 | Persist configured work-item provider kind, host, and project identity | 1 | `cargo test -p locus-core work_item::provider_configuration` |
| 3 | Implement the external-work-item provider registry and capability refusal | 1,2 | `cargo test -p locus-core work_item::adapter_selection` |
| 4 | Expose forge-provider issue trackers through the work-item provider port | 3, forge-providers:26 | `cargo test -p locus-core work_item::forge_adapter_bridge` |
| 5 | Implement Jira Cloud and Data Center work-item import | 3 | `cargo test -p locus-core work_item::jira_contract` |
...
| 8 | Refuse duplicate provider-identity import and open the existing task | 7 | `cargo test -p locus-core work_item::deduplicates_import` |
...
| 16 | Run conformance fixtures for GitHub, GitLab, Codeberg, both Bitbucket adapters, and Jira | 4,5 | `cargo test -p locus-core work_item::conformance` |
