...
**Implementation status:** the backend, persistence, lifecycle, policy, relay, workspace, config, audit, and verification paths are implemented with shim coverage. Task 5's template import was manually exercised against `sbx` v0.39.0 with a locally built agent image, and live sbx stdio, lifecycle, clone, push-back, and a real Pi RPC session passed. Task 17's ACP check remains harness-gated: the installed Pi 0.84.2 exposes `--mode rpc`/`--print` and rejects `--acp`, so no real Pi ACP session is available to run.
