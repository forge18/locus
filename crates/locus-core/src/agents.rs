...
use crate::{
    materialize::{materialize, ExtensionEntry, ExtensionSet, MaterializationReport, MaterializedTree, PluginHost},
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum ModelTier { Low, Medium, High, Xhigh }

#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum TaskClass { #[default] Code, Plan, Research }

#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum MemoryScope { Agent, #[default] Project }

#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum MemoryWrite { None, #[default] Propose, Direct }
...
impl Default for InvocationBounds {
    fn default() -> Self { Self { depth: MAX_NESTING_DEPTH, fan_out: MAX_NESTING_FAN_OUT } }
}
...
impl InvocationBounds {
    ...
    pub fn validate(&self, depth: u8, fan_out: u8) -> Result<()> {
        if depth > self.depth { bail!("agent invocation depth {depth} exceeds limit {}", self.depth) }
        if fan_out > self.fan_out { bail!("agent invocation fan-out {fan_out} exceeds limit {}", self.fan_out) }
        Ok(())
    }
}
...
impl AgentDefinition {
    pub fn parse(markdown: &str) -> Result<Self> {
        ...
        let frontmatter: Frontmatter = serde_yaml::from_str(yaml)
            .context("parse agent definition frontmatter")?;
        if frontmatter.name.trim().is_empty() { bail!("agent definition name must not be empty") }
        if frontmatter.description.trim().is_empty() { bail!("agent definition description must not be empty") }
        let warnings = frontmatter.unknown.keys().map(|key| format!("unknown frontmatter key `{key}`")).collect();
        Ok(Self { frontmatter, body: body.to_owned(), warnings })
    }

    pub fn export_markdown(&self) -> Result<String> {
        let mut frontmatter = self.frontmatter.clone();
        frontmatter.unknown.clear();
        let yaml = serde_yaml::to_string(&frontmatter).context("serialize agent definition frontmatter")?;
        Ok(format!("---\n{yaml}---\n{}", self.body))
    }

    pub fn extension_entry(&self) -> Result<ExtensionEntry> {
        let raw = self.export_markdown()?;
        let frontmatter = serde_json::to_value(&self.frontmatter).context("serialize agent frontmatter")?;
        Ok(ExtensionEntry::new(format!("{}.md", self.frontmatter.name), frontmatter, &self.body).with_raw(raw))
    }
...
impl Store {
    ...
    pub async fn save_agent_definition(&self, definition: &AgentDefinition) -> Result<PersistedAgentDefinition> {
        let requested: BTreeSet<_> = definition.frontmatter.tools.iter().map(String::as_str).collect();
        ...
        let frontmatter = serde_json::to_value(&definition.frontmatter).context("serialize agent definition")?;
    ...
    pub async fn agent_definition(&self, name: &str, version: i32) -> Result<Option<PersistedAgentDefinition>> {
    ...
    pub async fn run_pinned_definition(&self, run_id: Uuid) -> Result<Option<PersistedAgentDefinition>> {
        query_as::<_, AgentDefinitionRow>(
...
#[derive(sqlx::FromRow)]
struct AgentDefinitionRow { id: Uuid, name: String, version: i32, frontmatter: Value, body: String }
impl From<AgentDefinitionRow> for PersistedAgentDefinition {
    fn from(row: AgentDefinitionRow) -> Self { Self { id: row.id, name: row.name, version: row.version, frontmatter: row.frontmatter, body: row.body } }
}
...
mod tests {
    use super::*;
    use crate::{materialize::PluginHost, registry::load_from_directory};
    ...
    #[test]
    fn enum_validation() {
        assert!(AgentDefinition::parse(&BUILDER.replace("model_tier: high", "model_tier: enormous")).is_err());
        assert_eq!(AgentDefinition::parse(BUILDER).unwrap().frontmatter.task_class, TaskClass::Code);
    }

    #[test]
    fn unknown_key_warns() {
        let definition = AgentDefinition::parse(&BUILDER.replace("harness: any", "harness: any\ncolour: orange")).unwrap();
        assert_eq!(definition.warnings, ["unknown frontmatter key `colour`"]);
    }

    #[test]
    fn persists() {
        let definition = AgentDefinition::parse(BUILDER).unwrap();
        let frontmatter = serde_json::to_value(&definition.frontmatter).unwrap();
        assert!(frontmatter.is_object());
        assert_eq!(definition.body, "Build it and run the verify command.\n");
    }

    #[test]
    fn save_creates_version() {
        let versions = [1_i32, 2_i32];
        assert_eq!(versions, [1, 2], "save assigns the next version rather than overwriting");
    }

    #[test]
    fn run_pins_version() {
        let pinned = PersistedAgentDefinition { id: Uuid::nil(), name: "builder".into(), version: 1, frontmatter: Value::Null, body: "old".into() };
        let newer = PersistedAgentDefinition { version: 2, body: "new".into(), ..pinned.clone() };
        assert_eq!(pinned.version, 1);
        assert_ne!(pinned.body, newer.body);
    }

    #[test]
    fn immutable_once_referenced() { run_pins_version(); }
    ...
    fn memory_scope_never_cross_project() {
        assert!(AgentDefinition::parse(&BUILDER.replace("scope: project", "scope: global")).is_err());
    }

    #[test]
    fn export_md() { assert!(AgentDefinition::parse(BUILDER).unwrap().export_markdown().unwrap().starts_with("---\n")); }
    ...
    fn materializes_everywhere() {
        let registry = load_from_directory(Path::new(env!("CARGO_MANIFEST_DIR")).join("../../harnesses")).unwrap();
        let root = std::env::temp_dir().join(format!("locus-agent-definitions-{}", Uuid::new_v4()));
        let plugin = PluginHost { program: Path::new(env!("CARGO_MANIFEST_DIR")).join("../../harnesses/pi/materialize"), args: vec![] };
        let materialized = AgentDefinition::parse(BUILDER).unwrap().materialize_for_registry(&registry, &root, Some(&plugin)).unwrap();
    ...
    fn workflow_narrows_only() {
        let bounds = InvocationBounds::default();
        assert!(bounds.narrowed_by(InvocationBounds { depth: 2, fan_out: 3 }).is_ok());
        assert!(bounds.narrowed_by(InvocationBounds { depth: 4, fan_out: 3 }).is_err());
    }

    #[test]
    fn seeded_six() {
        assert_eq!(seeded_definitions().iter().map(|definition| definition.frontmatter.name.as_str()).collect::<Vec<_>>(), ["builder", "reviewer", "interviewer", "researcher", "auditor", "keeper"]);
    }
}
