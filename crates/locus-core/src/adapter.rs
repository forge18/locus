...
impl AdapterVersion {
    pub fn new(identity: impl Into<String>, version: impl Into<String>) -> Result<Self> {
        let adapter = Self { identity: identity.into(), version: version.into() };
...
impl<const N: usize> From<[AdapterVersion; N]> for AdapterRegistry {
    fn from(adapters: [AdapterVersion; N]) -> Self { Self(adapters.into_iter().collect()) }
}

impl AdapterRegistry {
    pub fn contains(&self, identity: &str, version: &str) -> bool {
        self.0.contains(&AdapterVersion { identity: identity.into(), version: version.into() })
    }
}
