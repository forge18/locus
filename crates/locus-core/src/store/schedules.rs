...
use sqlx::query;
use uuid::Uuid;
use crate::{runtime::dispatch::{ScheduleGuardrailOverrides, ScheduleMode}, store::Store};

fn mode_name(mode: ScheduleMode) -> &'static str { match mode { ScheduleMode::RunOnce => "once", ScheduleMode::Scheduled => "scheduled", ScheduleMode::Hold => "hold" } }
...
impl Store {
    pub async fn create_schedule(
        ...
        if cron_expression.trim().is_empty() { bail!("schedule expression is required"); }
    ...
    pub async fn record_schedule_execution(&self, id: Uuid, workflow_def_id: Uuid, scheduled_for: Option<String>, status: &str) -> Result<Uuid> {
