...
use crate::{
    ids::{ProjectId, RunId},
    services::wiki::{PageKind, WikiPage},
    store::Store,
};
...
impl Store {
    ...
    pub async fn append_wiki_revision(
        &self,
        page_id: Uuid,
        revision_id: Uuid,
        revision: u32,
        ...
        summary: &str,
        author_kind: &str,
        author_run: Option<RunId>,
    ) -> Result<()> {
        if body.trim().is_empty() {
            bail!("wiki revision body is required");
        }
    ...
    pub async fn wiki_project_id(&self, page_id: Uuid) -> Result<ProjectId> {
        let project_id: ProjectId = query("SELECT project_id FROM wiki.pages WHERE id = $1")
            .bind(page_id)
            .fetch_one(self.pool())
            .await
            .context("read wiki page project")?
            .try_get("project_id")?;
        Ok(project_id)
    }
}
