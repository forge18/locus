mod materialize {
    ...
    fn disabled_extensions_absent() {
        ...
        extensions.insert("rules", vec![ExtensionEntry::new("secret.md", json!({}), "never commit")]);
        ...
        let registry = load_from_directory(concat!(env!("CARGO_MANIFEST_DIR"), "/../../harnesses")).expect("registry");

        let (tree, _) = materialize_project(registry.by_name("claude").expect("claude"), &extensions, &scope, &root, None).expect("materialize");
